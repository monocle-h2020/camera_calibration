...
pol_file = argv[1]
files, angles = np.loadtxt(pol_file, dtype="S8", skiprows=2, unpack=True)
for filename, pol_angle in zip(files, angles):
    if pol_angle.decode == "DARK":
        continue
    filename = "test_files/gamma/"+filename.decode()+".dng"
    pol_angle = float(pol_angle)
    handle = filename.split("/")[-1].split(".")[0]
    polarisation_angle = pol_angle - polariser_angle
