...
for col in range(thickF.shape[1]):
    s = f"spectra/{col:03d}.png"
    for j in (0,1,2):
        plt.plot(thickF[:, col, j], c=RGB[j])
    plt.xlim(0, thickF.shape[0])
    plt.ylim(0,255)
    plt.title(s)
    plt.savefig(s)
...
folder = "spectra"
todo = "mencoder \"mf://./"+folder+"/*.png\" -mf w=600:h=400:fps=10:type=png -ovc copy -oac copy -o ./"+folder+".avi"
print(todo)
subprocess.call([todo], stdout=open(os.devnull, 'w'), shell=True)