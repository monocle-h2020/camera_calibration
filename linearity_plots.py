...
ax.errorbar(intensities, jmeans_to_plot[2], xerr=intensities_errors, fmt="bo", ms=3)
ax.set_xlim(-0.02, 1.02)
ax.set_ylim(0, 255*1.02)
ax.set_xticks(np.arange(0,1.2,0.2))
ax.set_yticks(np.arange(0, 255, 50))
ax2 = ax.twinx()
ax2.errorbar(intensities, means_to_plot[2], xerr=intensities_errors, fmt="ko", ms=3)
ax2.set_ylim(0, max_value*1.02)
ax2.locator_params(axis="y", nbins=5)
jpeglabel = ax.set_ylabel("JPEG value")
jpeglabel.set_color("b")
ax.tick_params(axis="y", colors="b")
ax2.set_ylabel("RAW value")
ax.set_xlabel("Relative incident intensity")
fig.savefig(results/"linearity/linearity_DNG_JPEG_singleB.pdf")
plt.close()
print("Single pixel comparison made")
