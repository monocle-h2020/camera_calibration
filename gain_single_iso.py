...
plt.plot(x, y, c='k', label=f"$G = {fit[0]:.2f}$ ADU/e$^-$")
plt.axhline(fit[1], c='k', ls="--", label=f"$RON = {RON:.2f}$ e$^-$")
plt.fill_between(x, y-yerr, y+yerr, color="0.5", label=f"$\sigma_G = {fiterr[0]:.2f}$\n$\sigma_R = {RONerr:.2f}$")
